...
export default class Mempool extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            error_value: "",
            last_updated: "",
            mempool_data_requests: [],
            data_request_fees: [],
            mempool_value_transfers: [],
            value_transfer_fees: []
        }
    ...
    getMempool() {
        DataService.getMempool()
        .then(response => {
            this.setState({
                loading: false,
                error_value: "",
                last_updated: TimeConverter.convertUnixTimestamp(response.last_updated, "hour"),
                mempool_data_requests: response.mempool_data_requests,
                data_request_fees: response.data_request_fees,
                mempool_value_transfers: response.mempool_value_transfers,
    ...
    generateMempoolChart(data, fees) {
    ...
    render() {
        ...
        if (error_value === "") {
            if (loading) {
                ...
                dataRequestPanel = this.generateMempoolChart(this.state.mempool_data_requests, this.state.data_request_fees);
                valueTransferPanel = this.generateMempoolChart(this.state.mempool_value_transfers, this.state.value_transfer_fees);
            }
        }
        else {
